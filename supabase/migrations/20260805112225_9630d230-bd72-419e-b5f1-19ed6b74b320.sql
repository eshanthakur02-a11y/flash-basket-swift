-- Shared eligibility + attribute logic for category filters
CREATE OR REPLACE FUNCTION public.category_filter_facets(
...
AS $function$
  ...
  prods AS (
    SELECT p.id, p.brand, p.unit, p.rating, p.mrp, p.category_id,
  ...
  subcats AS (
    SELECT DISTINCT pr.id AS product_id, c.id AS cat_id, c.name
    FROM prods pr
    JOIN public.product_categories pc ON pc.product_id = pr.id
    JOIN public.categories c ON c.id = pc.category_id
    WHERE _category_id IS NULL OR c.id <> _category_id
  )
  SELECT jsonb_build_object(
    ...
    'subcategories', COALESCE((
      SELECT jsonb_agg(x ORDER BY x->>'label')
      FROM (
        SELECT jsonb_build_object('id', cat_id, 'label', name, 'count', count(DISTINCT product_id)) AS x
        FROM subcats GROUP BY cat_id, name
      ) s3
    ), '[]'::jsonb),
    'ratings', COALESCE((
      ...
      FROM (
        ...
        WHERE EXISTS (SELECT 1 FROM prods WHERE rating >= v)
      ) r
    ), '[]'::jsonb),
    'discounts', COALESCE((
      ...
      FROM (
        SELECT jsonb_build_object('value', v, 'count', (
                 ...
                 WHERE mrp > 0 AND round((mrp - price) / mrp * 100) >= v)) AS x
        FROM (VALUES (10), (20), (30), (40), (50)) t(v)
        WHERE EXISTS (SELECT 1 FROM prods WHERE mrp > 0 AND round((mrp - price) / mrp * 100) >= v)
      ) d
    ), '[]'::jsonb)
  );
$function$;

REVOKE ALL ON FUNCTION public.category_filter_facets(text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.category_filter_facets(text, uuid) TO anon, authenticated, service_role;
...
CREATE OR REPLACE FUNCTION public.list_category_products(
  ...
  _min_discount int DEFAULT NULL,
  _sort text DEFAULT 'relevance',
  _limit int DEFAULT 60
)
RETURNS TABLE(id uuid, slug text, name text, unit text, price numeric, mrp numeric,
              image_url text, delivery_minutes integer, stock integer, rating numeric,
              category_id uuid, brand text)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
...
REVOKE ALL ON FUNCTION public.list_category_products(text, uuid, text, text[], text[], uuid[], numeric, numeric, numeric, int, text, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.list_category_products(text, uuid, text, text[], text[], uuid[], numeric, numeric, numeric, int, text, int) TO anon, authenticated, service_role;