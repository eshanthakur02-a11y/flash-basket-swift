...
AS $function$
BEGIN
  IF NOT (has_role(auth.uid(), 'admin'::app_role) OR EXISTS (SELECT 1 FROM public.support_agents sa WHERE sa.user_id = auth.uid() AND sa.is_active = true)) THEN
...
REVOKE ALL ON FUNCTION public.support_list_complaints() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.support_list_complaints() TO authenticated;
