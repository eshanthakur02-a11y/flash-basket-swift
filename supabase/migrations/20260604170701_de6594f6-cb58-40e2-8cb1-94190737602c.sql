CREATE OR REPLACE FUNCTION public.admin_create_shopkeeper(
  _user_email text,
  _shop_name text,
  _address text,
  _city text,
...
REVOKE ALL ON FUNCTION public.admin_create_shopkeeper(text,text,text,text,text,double precision,double precision,text,numeric) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_create_shopkeeper(text,text,text,text,text,double precision,double precision,text,numeric) TO authenticated;