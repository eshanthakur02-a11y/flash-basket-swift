...
export const Route = createFileRoute("/delivery/notifications")({
  head: () => ({ meta: [{ title: "Notifications — Delivery" }] }),
  component: Page,
});

function Page() {
  const { state, markNotificationRead, markAllRead } = useDemo();
  const list = state.notifications.filter((n) => n.role === "delivery");
  return (
    <DemoShell role="delivery" nav={DELIVERY_NAV}>
      <div className="px-4 md:px-6 py-5 max-w-3xl">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => markAllRead("delivery")}>Mark all read</Button>
        </div>
